...
protected:
	...
	void equilibradoAVL(Link raiz, bool & equi, int & altura) {
		int hIzq;
		int hDer;
		bool izq;
		bool der;
		if (raiz == nullptr) {
			...
			equilibradoAVL(raiz->left, izq, hIzq);
			equilibradoAVL(raiz->right, der, hDer);
			altura = std::max(hIzq, hDer) + 1;
			equi = izq && der && abs(hIzq - hDer) <= 1 && (raiz->cv.valor > raiz->left->cv.valor && raiz->cv.valor < raiz->right->cv.valor);
		}
	}
