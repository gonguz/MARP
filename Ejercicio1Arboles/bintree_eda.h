...
public:
   ...
   bool equilibrado() {
	   int altura;
	   bool equilibrado;
	    arbolEquilibrado(raiz, equilibrado, altura);
		return equilibrado;
   }
...
protected:
	void arbolEquilibrado(Link raiz, bool & equi, int & altura) {
		int hIzq;
		int hDer;
		bool izq;
		bool der;
		if (raiz == nullptr) {
			...
			arbolEquilibrado(raiz->left, izq, hIzq);
			arbolEquilibrado(raiz->right, der, hDer);
			altura = std::max(hIzq, hDer) + 1;
			equi = izq && der && abs(hIzq - hDer) <= 1;
		}
	}
